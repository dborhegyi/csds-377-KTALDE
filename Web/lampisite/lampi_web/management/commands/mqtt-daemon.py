...
class Command(BaseCommand):
    ...
    def _monitor_for_new_devices(self, client: Client, userdata: Any,
    ...
    def _monitor_broker_bridges(self, client: Client, userdata: Any,
                                message: MQTTMessage) -> None:
        self._monitor_for_new_devices(client, userdata, message)
        self._monitor_for_connection_events(client, userdata, message)

    def _monitor_for_connection_events(self, client: Client, userdata: Any,
                                       message: MQTTMessage) -> None:
        results = re.search(MQTT_BROKER_RE_PATTERN, message.topic.lower())
        if results is None:
            return
    ...
    def _monitor_lamp_state(self, client: Client, userdata: Any,
                            message: MQTTMessage) -> None:
