...
def unpack_POST_data(req: HttpRequest) -> tuple[str, str, str, str, str]:
    post_data = req.POST
...
def get_user_from_session_key(session_key: str) -> Optional[User]:
    try:
...
@csrf_exempt
def superuser(req: HttpRequest) -> HttpResponse:
    # do not allow any superusers
    return HttpResponseForbidden(None, content_type='application/json')


@csrf_exempt
def acl(req: HttpRequest) -> HttpResponse:
    if req.method == 'POST':
        if req.META['REMOTE_ADDR'] == '127.0.0.1':
            username, password, topic, acc, clientid = unpack_POST_data(req)
            # need to handle
